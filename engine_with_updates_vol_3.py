...
mag_model   = model.fit(x_train,y_train)    #only magnitude as feature
phase_model = model2.fit(x_train_phase,y_train)   #only phase as feature
...
y_prob       = mag_model.predict_proba(x_test)  #probabilities for each class (magnitude)
y_phase_prob = phase_model.predict_proba(x_test_phase)    #probabilities for each class (phase)
y_ensemble   = []
...
print("SVM: Combined Phase and Magnitude accuracy:")
...
plt.title('Max probability - SVM Models Confusion Matrix')
plt.show()
fig.savefig('Conf_Matrix_Combined_SVM_models.png',bbox_inches='tight',dpi=150)
...
print("XGBRegressor: Magnitude accuracy:")
print(round(sum(y_pred==y_test)/len(y_test)*100,1))
...
plt.title('XGBRegressor: Confusion Matrix of Magnitude')
plt.show()
fig.savefig('Conf_Matrix_XGBRegressor_AMPL.png',bbox_inches='tight',dpi=150)
...
xgb_model.fit(x_train_phase,y_train)

y_pred = xgb_model.predict(x_test_phase)
y_test = np.asarray(y_test)
print("XGBRegressor:  Phase  accuracy:")
print(round(sum(y_pred==y_test)/len(y_test)*100,1))
...
plt.title('XGBRegressor: Confusion Matrix of Phase')
plt.show()
fig.savefig('Conf_Matrix_XGBRegressor_PHS.png',bbox_inches='tight',dpi=150)
